...
export default function SessionDetailPage() {
  ...
  useEffect(() => {
    if (id && agent) {
      ...
      fetch(`${API_BASE_URL}/sessions/${id}?agent=${agent}`)
        .then((res) => res.json())
        .then((data) => {
          let evts: any[] = [];
  ...
  useEffect(() => {
    ...
    if (!cwd) return;
    fetch(`${API_BASE_URL}/config?project=${encodeURIComponent(cwd)}`)
      .then((r) => r.json())
        ...
        {artifacts.map((a, i) => (
          <div key={i} className="bg-slate-900/60 border border-slate-800 rounded-xl overflow-hidden shadow-lg group text-[11px]">
            <div className="px-3 py-2 border-b border-slate-800 bg-slate-950/40 flex items-center justify-between">
               ...
               <a 
                 href={`${API_BASE_URL}/artifacts?path=${encodeURIComponent(a.path)}`} 
                 download={a.name}
            ...
            <div className="p-3">
               {a.type === 'video' && (
                 <video controls className="w-full rounded-lg shadow-inner bg-black aspect-video">
                   <source src={`${API_BASE_URL}/artifacts?path=${encodeURIComponent(a.path)}`} type="video/mp4" />
                   Your browser does not support the video tag.
                 </video>
               )}
               {a.type === 'image' && (
                 <img 
                    src={`${API_BASE_URL}/artifacts?path=${encodeURIComponent(a.path)}`} 
                    alt={a.name} 
...
function ArtifactViewer({ path }: { path: string }) {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/artifacts?path=${encodeURIComponent(path)}`)
      .then(res => res.text())
      .then(t => {
        setContent(t);
        setLoading(false);
